...
// this test uses server-stored TXserver value instead of saving it to local file
    ...
    if (cm.getIsExtracted()) {
        ...
        if (cm.getIsAck()) {
            ...
            if (!cm.getIsNotification()) {
                ...
                // send server-stored TXserver value as data to this ack
                var storedTXserver = { 'TXserver': TXserver };
                jsAck.setDataAsObject(storedTXserver);
                    ...
                    if(!authorized) {
                        ...
                        if("result" in authReply && authReply.result == 0 && "type" in authReply && authReply.type == 'authentication_response') {
                            ...
                            if(cm.getIsSync()) {
                                ...
                                // lets get TXserver value from server!
                                TXserver = authReply.TXserver;
                            }
